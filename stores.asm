...
/********************************************************
* stores  -- program to load and store 		         *
*																										   *
*                                                      *
...
********************************************************/
	...
	.data
Array:  .space 8	# 8 bytes of an integer-array 4 bts for each integer
	.text

	la $t0, Array  #write address of array into register $t0
	li $t1, 2      #load number 2 into register $t1
	sw $t1, 0($t0) #store first element into the array cell
	li $t1, 3      #load number 3 into register $t1
	...
	nop
	nop
	add $t6, $t5, $t2  #add $t5 and %t2 and store it in $t6
	nop
	nop
